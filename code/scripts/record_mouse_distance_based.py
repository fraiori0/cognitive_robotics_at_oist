...
class MouseRecorder:
    def __init__(
        ...
        # scale the distance threshold to the canvas size
        self.distance_threshold = distance_threshold * max(
    ...
    def showxy(self):
        if self.update_position and self.last_event:
            ...
            # check if the distance to the last point is greater than the threshold
            if len(self.recordings[-1]) == 0:
                distance = np.inf
            else:
            ...
            if distance > self.distance_threshold:
                ...
                timestamp = time.time()
                # remove the oldest point if we are at the maximum length
