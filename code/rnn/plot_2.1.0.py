...
fig_suffix = f"_{data_name}_{hidden_size}_{test_name}"
if random_hidden_states:
    fig_suffix += "_randh0"
if backprop_initial_states:
    fig_suffix += "_backprop"

fig_suffix += f"_initseq{initial_seq_length}"
    ...
    with open(os.path.join(model_path, f"{model_name}_h0.pkl"), "rb") as f:
        ...
        for k in hidden_states.keys():
            if len(hidden_states[k]["h0"].shape) == 1:
                ...
                hidden_states[k]["x0"] = hidden_states[k]["x0"][None, :]
...
# 111111111111111111111111111111111111exit()
...
for k in ps_dict.keys():
    ...
    # or the one associated to the closest first point of the training sequence
    else:
        # find the closest point
        dist = np.linalg.norm(
            ps_dict[k]["x"][:, 0, None] - hidden_states[k]["x0"][None, :], axis=-1
        )
        idx = np.argmin(dist, axis=-1)
        print(k, hidden_states[k]["h0"][idx].shape)
        H0.append(hidden_states[k]["h0"][idx])
...
if backprop_initial_states:
    for i in range(n_gradient_updates_hidden):
        ...
        H0 = np.clip(
            H0,
            model_info_dict["h0_min_activation"],
            model_info_dict["h0_max_activation"],
        )
...
for k in h_traj_dict.keys():
    idx = np.where(IDS == ps_dict[k]["id"])[0]
    h0_tmp = H0[idx]
    h0_pca[k] = (h0_tmp[..., None, :] * pca_components_dict[k]).sum(axis=-1)
...
# NOTE, the string is to be completed to add transparency
my_colors = [
    "rgba(0, 157, 255",
    "rgba(255, 0, 0",
    "rgba(0, 255, 0",
